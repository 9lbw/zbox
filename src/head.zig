...
pub fn head_main(args: []const []const u8) !void {
    ...
    if (args.len > 0 and args[0].len > 1 and args[0][0] == '-') {
        if (std.mem.eql(u8, args[0], "--help")) {
            try stdout.print("Usage: head [OPTION]... [FILE]...\n", .{});
            try stdout.print("Print the first 10 lines of each FILE to standard output.\n", .{});
            try stdout.print("With more than one FILE, precede each with a header giving the file name.\n", .{});
            try stdout.print("\n", .{});
            try stdout.print("  -n, --lines=[-]NUM       print the first NUM lines instead of the first 10\n", .{});
    ...
    if (num_lines < 0) {
        try stderr.print("head: invalid number of lines: '{d}'\n", .{num_lines});
        std.process.exit(1);
    }

    // If no files specified, read from stdin
    if (file_args.len == 0) {
        try head_file(allocator, null, @intCast(num_lines), false);
        return;
    }
    ...
    for (file_args, 0..) |filename, i| {
        ...
        head_file(allocator, filename, @intCast(num_lines), file_args.len > 1) catch |err| switch (err) {
            error.FileNotFound => {
                try stderr.print("head: cannot open '{s}' for reading: No such file or directory\n", .{filename});
                std.process.exit(1);
            },
...
fn head_file(allocator: std.mem.Allocator, filename: ?[]const u8, num_lines: u32, _: bool) !void {
    const stdout = std.io.getStdOut().writer();
